...
def eval_run(df, metric):
    # 更优雅的做法
    m, n = metric.split('@') # `recall@10` -> m = 'recall', n = '10'
    return eval(m)(df, int(n))

metric4dataset = {'Equity':['recall@3','recall@10','mrr@10','ndcg@10'], 
            'Tafeng':['recall@10','recall@20','mrr@20','ndcg@20'],
            'Taobao':['recall@20','mrr@20','ndcg@20']
            }
def __main__():
    """ 测试上述代码
    The main function that iterates through datasets and performs eval_run on each combination of d and m.
    需要确保传入的df类型为pd.DataFrame。'V_u'和'R_u'两列?
    """
    # 跑完一个算法后，得到的result，用于计算metric，在表格里就一行
    df = pd.DataFrame()
    metric_df = pd.DataFrame()
    for d in metric4dataset:
        for m in metric4dataset[d]:
            metric_df.loc[d, m] = eval_run(df, m)
    
    metric_df.to_csv()      # 生成csv
    metric_df.to_latex()    # 最后生成latex表格
